...
<%
 ...
 try{
  ...
  pstmt=conn.prepareStatement("select * from database.final where id=?"); //prepareStatement는 Statement와 다르게 ?로 지정된 값을 필요 할 때 마다 이용할수있다.
  ...
  if(rs.next()){  
   if(pass.equals(rs.getString("pw"))){ 
    ...
    out.println("<script>");
    out.println("location.href='list.jsp'"); //저장된 정보와 아이디와 비밀번호가 일치하면 main.jsp로 보낸다. main.jsp는 정보가 출력되는 창이다.
    out.println("</script>");
   }
  }
  out.println("<script>");
